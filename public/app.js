...
// Default Source Location (Kuala Lumpur)
...
const CITY_PRESETS = Object.freeze([
  { name: 'Kuala Lumpur, Malaysia', lat: 3.139, lng: 101.6869, country: 'MY' },
...
const state = {
  ...
  customSource: null,  // Custom source location {lat, lng, city, country}
  sourceMarker: null  // Reference to source marker
};
...
function setupEventListeners() {
  ...
  if (sourceLocationToggle) sourceLocationToggle.addEventListener('click', showSourceLocationModal);
  if (modalCloseBtn) modalCloseBtn.addEventListener('click', hideSourceLocationModal);
  if (saveSourceBtn) saveSourceBtn.addEventListener('click', saveCustomSource);
  if (resetSourceBtn) resetSourceBtn.addEventListener('click', resetSourceToDefault);
  if (cityPreset) cityPreset.addEventListener('change', handleCityPresetChange);

  // Close modal when clicking backdrop
  if (modal) {
    modal.addEventListener('click', (e) => {
      if (e.target === modal || e.target.classList.contains('modal-backdrop')) {
        hideSourceLocationModal();
      }
    });
  }
...
function loadPreferences() {
  try {
    ...
    const savedSource = localStorage.getItem('customSource');
    if (savedSource) {
      try {
        state.customSource = JSON.parse(savedSource);
        // Note: updateSourceLocationButton removed since sidebar button was removed
      } catch (error) {
        console.warn('Failed to parse saved source location:', error);
...
function addSourceMarker() {
  ...
  state.sourceMarker = new maplibregl.Marker({ color: '#f6ad55' })
    ...
    .setPopup(new maplibregl.Popup().setText(`DNS Source: ${source.city}`))
    .addTo(state.map);
}
...
function handleDNSQuery(event) {
  ...
  // Use custom source location instead of server-provided source
  if (event.destination && state.activeArcs.length < CONFIG.MAX_CONCURRENT_ARCS) {
    const customSource = getActiveSource();
    createArc(customSource, event.destination, event.data);
  }
...
/**
 * Get the active source location (custom or default)
 * @returns {Object} Source location {lat, lng, city, country}
 */
function getActiveSource() {
  return state.customSource || DEFAULT_SOURCE;
}

/**
 * Show the source location modal
 */
function showSourceLocationModal() {
  const modal = document.getElementById('source-location-modal');
  if (!modal) return;

  // Populate city presets dropdown
  const cityPreset = document.getElementById('city-preset');
  if (cityPreset && cityPreset.options.length === 1) {
  ...
  // Populate current values if custom source exists
  const activeSource = getActiveSource();
  document.getElementById('source-lat').value = activeSource.lat;
  document.getElementById('source-lng').value = activeSource.lng;
  document.getElementById('source-city').value = activeSource.city;
...
/**
 * Hide the source location modal
 */
function hideSourceLocationModal() {
  const modal = document.getElementById('source-location-modal');
  if (modal) {
 ...
 */
function handleCityPresetChange(event) {
  ...
  try {
    ...
    document.getElementById('source-lat').value = city.lat;
    document.getElementById('source-lng').value = city.lng;
    document.getElementById('source-city').value = city.name.split(',')[0];
...
function validateCoordinates(lat, lng) {
  ...
  // Warning for (0,0) unless intentional
  if (lat === 0 && lng === 0) {
    return { valid: true, message: 'Warning: Setting source to (0,0) - Null Island' };
  }
 ...
 * @param {boolean} isError - Whether this is an error or success
 */
...
/**
 * Save custom source location
 */
function saveCustomSource() {
  const latInput = document.getElementById('source-lat');
  const lngInput = document.getElementById('source-lng');
  ...
  const city = cityInput.value.trim() || 'Custom Location';
  ...
  // Show warning message if coordinates are (0,0)
  if (validation.message) {
    showValidationMessage(validation.message, false);
  }

  // Create custom source object
  const customSource = {
    lat,
    lng,
    city,
  ...
  // Save to localStorage
  try {
    localStorage.setItem('customSource', JSON.stringify(customSource));
  } catch (error) {
    console.error('Failed to save custom source to localStorage:', error);
    showValidationMessage('Failed to save location', true);
    return;
  }

  // Update UI
  updateSourceMarker(lat, lng, city);
  updatePulseSource(lat, lng);

  // Show success message
  showValidationMessage(`Source location updated to ${city}`, false);
...
/**
 * Reset source to default (Kuala Lumpur)
 */
function resetSourceToDefault() {
  // Clear custom source from state
  state.customSource = null;

  // Remove from localStorage
  try {
    localStorage.removeItem('customSource');
  } catch (error) {
    console.error('Failed to remove custom source from localStorage:', error);
  }

  // Update UI to default
  updateSourceMarker(DEFAULT_SOURCE.lat, DEFAULT_SOURCE.lng, DEFAULT_SOURCE.city);
  updatePulseSource(DEFAULT_SOURCE.lat, DEFAULT_SOURCE.lng);

  // Update modal fields
  document.getElementById('source-lat').value = DEFAULT_SOURCE.lat;
  document.getElementById('source-lng').value = DEFAULT_SOURCE.lng;
  document.getElementById('source-city').value = DEFAULT_SOURCE.city;

  // Reset dropdown
  const cityPreset = document.getElementById('city-preset');
  if (cityPreset) {
    cityPreset.selectedIndex = 0;
  }

  // Show success message
  showValidationMessage('Reset to default location: Kuala Lumpur', false);
 ...
 */
function updateSourceMarker(lat, lng, city) {
  if (!state.map || !state.sourceMarker) return;

  try {
    state.sourceMarker.setLngLat([lng, lat]);
    state.sourceMarker.setPopup(new maplibregl.Popup().setText(`DNS Source: ${city}`));
  } catch (error) {
    console.error('Error updating source marker:', error);
  }
}

/**
 * Update pulse source position
 * @param {number} lat - Latitude
 * @param {number} lng - Longitude
 */
function updatePulseSource(lat, lng) {
  if (!state.map) return;

  try {
    const source = state.map.getSource('pulse-source');
    if (source) {
      source.setData({
        type: 'FeatureCollection',
        features: [{
    ...
    console.error('Error updating pulse source:', error);
  }
}
